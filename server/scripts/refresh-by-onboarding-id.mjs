#!/usr/bin/env node
// Like refresh-admin-company-overview.mjs but takes an explicit
// OnboardingCompany _id so we can target a specific signup when the
// company name matches multiple records (e.g., two founders signed up the
// same brand). Usage:
//
//   APPLY=1 MONGODB_URI=... node refresh-by-onboarding-id.mjs <onboarding_company_id>
...
if (!onboardingId) {
  console.error('Usage: refresh-by-onboarding-id.mjs <onboarding_company_id>');
  process.exit(1);
}
...
(async () => {
  ...
  const company = await onb.collection('onboardingcompanies').findOne({ _id: new ObjectId(onboardingId) });
  if (!company) { console.error(`OnboardingCompany ${onboardingId} not found.`); process.exit(1); }
  console.log(`Onboarding company: "${company.companyName}" · vertical=${company.vertical} · launched=${!!company.phaseBCompletedAt}`);

  const docs = await onb.collection('onboardingdocs').find({ companyId: company._id }).toArray();
  console.log(`Loaded ${docs.length} docs (${docs.map((d) => d.kind).join(', ') || 'none'})`);

  const adminCompany = await adm.collection('companies').findOne({ onboarding_company_id: company._id });
  if (!adminCompany) { console.error(`No admin company linked to onboarding ${onboardingId}`); process.exit(1); }
  ...
  console.log('  products count:', ci.primary_products_and_services.length);
  console.log('  differentiators count:', ci.key_differentiators.length);
  console.log('  competitors count:', ci.competitors.length);
  console.log('  objections count:', ci.common_objections.length);
  ...
  const r = await adm.collection('companies').updateOne(
